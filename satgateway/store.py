...
class PaymentStore:
    ...
    def save(self, req: PaymentRequest, csrf_token: Optional[str] = None):
        """Insert a new payment record. Does NOT overwrite existing rows."""
        self._conn().execute(
            """
    ...
    def update_csrf(self, payment_id: str, csrf_token: str):
        """Update only the CSRF token — never touches consumed or other fields."""
        self._conn().execute(
            "UPDATE payments SET csrf_token = ? WHERE id = ?",
            (csrf_token, payment_id),
        )
