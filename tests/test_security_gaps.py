...
# Set required env var for tests that trigger key resolution
os.environ.setdefault("SATGATEWAY_KEY", "test-key-do-not-use-in-production")
...
# ---------------------------------------------------------------------------
# DeepSeek audit fixes
# ---------------------------------------------------------------------------

class TestUpdateCsrf:
    """Verify update_csrf does NOT reset consumed flag (anti double-spend)."""

    def test_update_csrf_preserves_consumed(self):
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
            db_path = f.name
        try:
            store = PaymentStore(db_path=db_path)
            ...
            store.save(req)
            # Consume the payment
            consumed = store.verify_and_consume("csrf-test")
            assert consumed is not None

            # Update CSRF token (simulating paywall page visit)
            store.update_csrf("csrf-test", "new-csrf-token")

            # Verify still consumed — double-spend protection intact
            second = store.verify_and_consume("csrf-test")
...
class TestResourceUrlValidation:
    ...
    def test_reject_javascript_scheme(self):
        from satgateway.middleware import _validate_resource_url
        from fastapi import HTTPException
        with pytest.raises(HTTPException) as exc:
            _validate_resource_url("javascript:alert(1)")
        assert exc.value.status_code == 400
        assert "javascript" in exc.value.detail.lower()

    def test_reject_data_scheme(self):
        from satgateway.middleware import _validate_resource_url
        from fastapi import HTTPException
        with pytest.raises(HTTPException) as exc:
            _validate_resource_url("data:text/html,<script>alert(1)</script>")
        assert exc.value.status_code == 400

    def test_allow_http(self):
        from satgateway.middleware import _validate_resource_url
        assert _validate_resource_url("https://example.com") == "https://example.com"

    def test_allow_relative(self):
        from satgateway.middleware import _validate_resource_url
        assert _validate_resource_url("/api/content") == "/api/content"

    def test_allow_empty(self):
        from satgateway.middleware import _validate_resource_url
        assert _validate_resource_url("") == ""


class TestResolveApiKey:
    """Verify startup fails when SATGATEWAY_KEY is missing."""

    def test_missing_key_raises(self):
        from satgateway.middleware import _resolve_api_key
        # Clear env temporarily
        old = os.environ.pop("SATGATEWAY_KEY", None)
        try:
            with pytest.raises(RuntimeError) as exc:
                _resolve_api_key()
            assert "SATGATEWAY_KEY" in str(exc.value)
        finally:
            if old is not None:
                os.environ["SATGATEWAY_KEY"] = old

    def test_explicit_key_returns(self):
        from satgateway.middleware import _resolve_api_key
        os.environ["SATGATEWAY_KEY"] = "my-secret-key"
        assert _resolve_api_key() == "my-secret-key"


class TestClampInvoiceWithUrlValidation:
    """Verify _clamp_invoice_input rejects bad resource_url schemes."""

    def test_clamp_rejects_javascript_url(self):
        from satgateway.middleware import _clamp_invoice_input
        from fastapi import HTTPException
        with pytest.raises(HTTPException) as exc:
            _clamp_invoice_input({"amount_sats": 100, "resource_url": "javascript:alert(1)"})
        assert exc.value.status_code == 400
