    ...
    function renderPaywall(payment) {
        container.innerHTML = `
            <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 1.5rem; text-align: center; max-width: 360px; margin: 0 auto;">
                ...
                <div style="font-size: 2rem; font-weight: 700; margin: 0.5rem 0;">${payment.amount_sats.toLocaleString()} <span style="font-size: 1rem; color: #8b949e;">sats</span></div>
                <div style="margin: 1rem 0;">
                    <img src="${baseUrl}/payments/qr/${payment.payment_id}" alt="QR Code" style="max-width: 200px; border-radius: 8px;">
                </div>
                <button onclick="copyInvoice('${payment.invoice}')" style="background: #F7931A; color: #000; border: none; padding: 0.6rem 1.2rem; border-radius: 8px; font-weight: 600; cursor: pointer;">Copy Invoice</button>
                <div style="font-size: 0.75rem; color: #484f58; word-break: break-all; margin-top: 1rem;">${payment.invoice}</div>
    ...
    function renderContent(data) {
        container.innerHTML = `
            <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 1.5rem; text-align: center; max-width: 360px; margin: 0 auto;">
                ...
                <pre style="text-align: left; background: #0d1117; padding: 1rem; border-radius: 8px; overflow-x: auto; font-size: 0.8rem; color: #e6edf3;">${JSON.stringify(data, null, 2)}</pre>
            </div>
        `;
    }
